...
use crate::{ErrorType, repo_file_manager::{get_link, get_file_content, get_delta_files_paths}, Links, vcs_file_manager::{get_branch_hash, get_commits_path, get_files_dir_from_commit}, file_factory::make_file_from_hash};

pub fn merge(branch_name: String) -> Result<(), ErrorType>{

    let cur_dir = std::fs::canonicalize("./").unwrap();
    let vcs_path = get_link(Links::Vcs, cur_dir.clone())?;
    let cur_head_commit_hash = get_file_content(get_link(Links::Head, vcs_path.clone()).unwrap())?;
    let last_master_commit_hash = get_file_content(get_link(Links::Master, vcs_path.clone()).unwrap())?;
    ...
    get_files_dir_from_commit(&vcs_path, &last_general_commit_hash, &mut general_files_paths, &mut general_files_hashes)?;

    let mut branch_files_paths: Vec<PathBuf> = vec![];
    let mut branch_files_hashes: Vec<String> = vec![];
    get_files_dir_from_commit(&vcs_path, &branch_commits_hashes.last().unwrap(), &mut branch_files_paths, &mut branch_files_hashes)?;

    let mut master_files_paths: Vec<PathBuf> = vec![];
    let mut master_files_hashes: Vec<String> = vec![];
    get_files_dir_from_commit(&vcs_path, &master_commits_hashes.last().unwrap(), &mut master_files_paths, &mut master_files_hashes)?;

    let general_to_branch = get_delta_files_paths(&general_files_paths, &general_files_hashes, &branch_files_paths, &branch_files_hashes);
    let general_to_master = get_delta_files_paths(&general_files_paths, &general_files_hashes, &master_files_paths, &master_files_hashes);

    let mut both_mod: Vec<PathBuf> = vec![];

    for path in general_to_branch.1.iter().enumerate() {
        if general_to_branch.0[path.0] == "deleted" {
            continue;
        }

        let mut master_iter = general_to_master.1.iter();
        let index = master_iter.position(|x| *x == *path.1);

        if index.is_some() {
            if general_to_branch.2[path.0] != general_to_master.2[index.unwrap()] {
                both_mod.push(general_to_branch.1[path.0].clone());
            }
        }
    }

...
// Если в сигнатуре функции не указан тип возвращаемого значения, как тут у main,
// то можно считать, что возвращается тип `()`.
// То есть `fn main() -> ();` и `fn main();` это одно и то же.
fn explain_returns() {
    ...
    // и это будет работать, потому что это *последнее* выражение в функции main
    // и тип результата совпадает с типом возвращаемого значения функции main - `()`.
    println!("Hello, world!")
}
...
fn explain_scopes_and_blocks() {
    ...
    // Переменная `x` живет до конца функции main.
    let x = rand();

    // Это новый блок кода, который живет внутри блока функции main
